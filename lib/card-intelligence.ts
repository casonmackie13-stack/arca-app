...
export type CardAutofillRequest = { image_url?: string; image_base64?: string; image_mime_type?: "image/jpeg" | "image/png" | "image/webp"; archive_path?: string };
export type CardAutofillResponse = { card: AutofillCard; sales_query: string; training_event_id?: string | null; archive_path?: string | null };
export type SaleResult = { title: string; price: string; currency: string; sale_date: string; source: string; sale_type: "auction" | "buy_it_now" | "best_offer" | "unknown"; condition: string; grade_company: string; grade: string; url: string; confidence: number };
