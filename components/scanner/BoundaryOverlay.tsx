...
export default function BoundaryOverlay({ type, overlayRef }: { type: ScanType; overlayRef: RefObject<HTMLDivElement | null> }) {
  ...
  return <div className="pointer-events-none absolute inset-0 flex items-center justify-center px-6 py-24">
    ...
    >
      <span className="absolute -left-1 -top-1 h-7 w-7 rounded-tl-[1.35rem] border-l-4 border-t-4 border-white" />
      <span className="absolute -right-1 -top-1 h-7 w-7 rounded-tr-[1.35rem] border-r-4 border-t-4 border-white" />
      <span className="absolute -bottom-1 -left-1 h-7 w-7 rounded-bl-[1.35rem] border-b-4 border-l-4 border-white" />
      <span className="absolute -bottom-1 -right-1 h-7 w-7 rounded-br-[1.35rem] border-b-4 border-r-4 border-white" />
